...
- **修复「抓取卡在第 1 页 +0 条」**：ProductHunt 排行榜按 **美国太平洋时间（America/Los_Angeles）** 每天 0 点换日。
  之前程序用本机时间算「今天」，中国用户（UTC+8）上午点「抓取今天」时，该日期在太平洋时间还没开始，
  榜单自然是空的，于是卡在「第 1 页: +0 条」。现在「今天 / 本周 / 本月」一律按太平洋时间计算（日榜/周榜/月榜 GUI 和 CLI 均已修正）。
- **自动回退**：日榜「今天」若仍无数据，可自动改抓前一天（设置里可开关，默认开启）。
- **抓取诊断**：返回 0 条时日志会说明原因（空榜 / GraphQL 接口变动 / 全被广告过滤），便于排查。
- **「设置」面板**：GUI 新增「设置」按钮，可配置——
  - 启用/关闭自动浏览器验证、手动指定浏览器路径、浏览器验证超时；
  - 请求超时、HTTP/HTTPS 代理；
  - 默认导出目录、抓完是否自动弹导出窗、日榜自动回退开关。
  所有设置保存在 `~/.ph_scraper/config.json`。
- Windows 打包已内置 `tzdata`，确保 `zoneinfo` 时区库在 exe 中可用。
